...
aria2_port = os.environ.get('aria2_port')
aria2_secret = os.environ.get('aria2_secret')
qwbotkey = os.environ.get('qwbotkey')
...
class TianyiDownloader:
    def __init__(self):
        ...
        self.aria2 = aria2p.API(
            aria2p.Client(
                host=aria2_host,
                port=aria2_port,
                secret=aria2_secret  # 如果没有设置RPC密码则留空
            )
        )
    ...
    def send_message(self, name, size):    
    ...
    def download_with_aria2(self, download_url, save_path):
        """推送到 Aria2 下载"""
        try:
            # 创建保存目录（Aria2不会自动创建）
            ...
            print(f"✓ 任务已推送 (GID: {download.gid}) | 保存路径: {save_path}")
            return download.gid
        except Exception as e:
            print(f"✗ Aria2 推送失败: {str(e)}")
            return None

    def check_download_status(self, gid):
        """检查下载状态"""
        try:
            download = self.aria2.get_download(gid)
            return {
                "status": download.status,
                "progress": f"{download.progress:.2f}%",
                "speed": f"{download.download_speed/1024:.2f} KB/s",
                "completed": download.is_complete
            }
    ...
    def download_share_file(self, share_url, save_dir):
        ...
        for fileListItem in self.sort_file_list(fileList):
            ...
            if os.path.exists(save_path):
                ...
                # 推送下载任务
                gid = self.download_with_aria2(download_url, save_path)

                # 发送通知    
                self.send_message(fileListItem['name'], self._format_size(fileListItem['size']))

                # 监控进度
                if gid:
                    while True:
                        status = downloader.check_download_status(gid)
                        print(f"状态: {status['status']} | 进度: {status['progress']} | 速度: {status['speed']}")
...
if __name__ == "__main__":
    ...
    try:
        for share_item in share_list:
          ...
          save_dir = f"/downloads/{share_item['name']}"
